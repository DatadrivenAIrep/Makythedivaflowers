...
- [ ] **Step 2: Verify the card background asset exists**

The artwork at `public/print/card-bg-default.png` was generated by Task 12 (run before this plan's autonomous execution). Verify it exists; if it doesn't, fall back to a 1×1 transparent placeholder so tests can run.

```bash
mkdir -p public/print
if [ ! -f public/print/card-bg-default.png ]; then
...
**Important:** do NOT overwrite the file if it already exists — the real artwork is committed there.

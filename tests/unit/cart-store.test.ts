...
describe("cartStore", () => {
  ...
  it("adds a new line", () => {
    useCartStore.getState().add({ kind: "catalog", productId: "p1", variantId: "v1", addOnIds: [], qty: 1 });
    expect(useCartStore.getState().lines).toHaveLength(1);
  });

  it("merges qty when adding the same product+variant twice", () => {
    const { add } = useCartStore.getState();
    add({ kind: "catalog", productId: "p1", variantId: "v1", addOnIds: [], qty: 1 });
    add({ kind: "catalog", productId: "p1", variantId: "v1", addOnIds: [], qty: 2 });
    expect(useCartStore.getState().lines).toHaveLength(1);
  ...
  it("treats different variants as separate lines", () => {
    const { add } = useCartStore.getState();
    add({ kind: "catalog", productId: "p1", variantId: "v1", addOnIds: [], qty: 1 });
    add({ kind: "catalog", productId: "p1", variantId: "v2", addOnIds: [], qty: 1 });
  ...
  it("count sums qty across lines", () => {
    ...
    add({ kind: "catalog", productId: "p1", variantId: "v1", addOnIds: [], qty: 2 });
    add({ kind: "catalog", productId: "p2", variantId: "v1", addOnIds: [], qty: 3 });
    expect(count()).toBe(5);
  });
